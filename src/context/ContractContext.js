...
export function ContractProvider({ children }) {
  ...
  useEffect(() => {
    ...
    setWaterExchangeRecordContract(
      new ethers.Contract(CONTRACT_ADDRESS.WATER_EXCHANGE_RECORD, waterExchangeRecordContractJson.abi)
    );
    setWaterPhMeasurementContract(
      new ethers.Contract(CONTRACT_ADDRESS.WATER_PH_MEASUREMENT, waterPHMeasurementContractJson.abi)
    );
    setWaterTANMeasurementContract(
      new ethers.Contract(CONTRACT_ADDRESS.WATER_TAN_MEASUREMENT, waterTANMeasurementContractJson.abi)
    );
    setWaterTempMeasurementContract(
      new ethers.Contract(CONTRACT_ADDRESS.WATER_TEMP_MEASUREMENT, waterTempMeasurementContractJson.abi)
    );
  }, []);
